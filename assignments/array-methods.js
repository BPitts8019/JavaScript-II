...
const runners = [{"id":1,"first_name":"Charmain","last_name":"Seiler","email":"cseiler0@wired.com","shirt_size":"2XL","shirt_size":"Divanoodle","donation":75},
...
console.log("==== Challenge 2: Use .map() ====\n", allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
largeShirts = runners.filter(runner => runner.shirt_size === "L");
console.log("==== Challenge 3: Use .filter() ====\n", largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal = runners.reduce((total, runner) => total + runner.donation, 0);
console.log("==== Challenge 4: Use .reduce() ====\n", ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
console.log("==== Challenge 5: Be Creative ====");
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
// The event director would like to send a thank you note to each runner. Compile a list of runner's names with their email addresses.
const emailList = runners.map(runner => `${runner.first_name} ${runner.last_name} - ${runner.email}`);
console.log("==== Email List ====\n", emailList);

// Problem 2
// Make a list of all the runners from companies that donated between 100 and 200 dollars
let highTierRunners = runners.filter(runner => runner.donation > 100 && runner.donation < 200);
highTierRunners = highTierRunners.map(runner => `${runner.first_name} ${runner.last_name}: ${runner.donation}`);
console.log("==== Donations Between 100 and 200 Dollars ====\n", highTierRunners);

// Problem 3
// Group the runners by shirt size.
const groupedRunners = runners.reduce((groups, runner) => {
   if (!groups[runner.shirt_size]) {
      groups[runner.shirt_size] = [];
   }

   groups[runner.shirt_size].push(`${runner.first_name} ${runner.last_name}`);

   return groups;
}, {});
console.log("==== Runners Grouped by Shirt Size ====\n", groupedRunners);